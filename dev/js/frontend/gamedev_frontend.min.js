...
jQuery(document).ready(function ($) {
  ...
  function preload() {
    ...
    this.load.tilemapTiledJSON('level1', gamedevPhpVariables.GAMEDEV_ROOT_IMG_GAMEASSETS_ENVIRONMENTS_URL + "overworld.json"); //THIS WORKS!!!!!!!!!
  ...
  function create() {
    ...
    var tileset = map.addTilesetImage('overworld', 'gameTiles');
    var backgroundLayer = map.createStaticLayer('backgroundLayer', tileset);
    var worldLayer = map.createStaticLayer("worldLayer", tileset, 0, 0);
    var aboveLayer = map.createStaticLayer("aboveLayer", tileset, 0, 0);
    ...
    this.player = this.physics.add.sprite(spawnPoint.x, spawnPoint.y, "atlas", "misa-front").setSize(10, 16).setOffset(3, 10); // Watch the this.player and worldLayer for collisions, for the duration of the scene:

    this.physics.add.collider(this.player, worldLayer); // Create the this.player's walking animations from the texture atlas. These are stored in the global
    ...
    this.input.keyboard.once("keydown_D", function (event) {
      // Turn on physics debugging to show this.player's hitbox
      _this.physics.world.createDebugGraphic(); // Create worldLayer collision graphic above the this.player, but below the help text


      var graphics = _this.add.graphics().setAlpha(0.75).setDepth(20);
  ...
  function update(time, delta) {
    ...
    var speed = 175;
    var prevVelocity = this.player.body.velocity.clone(); // Stop any previous movement from the last frame

    this.player.body.setVelocity(0); // Horizontal movement
    ...
    this.player.body.velocity.normalize().scale(speed); // Walking aniations.
    ...
    this.input.keyboard.once("keyup_UP", function (event) {
      _this2.player.anims.stop();

      _this2.player.setTexture("atlas", "misa-back");
    });
    this.input.keyboard.once("keyup_DOWN", function (event) {
      _this2.player.anims.stop();

      _this2.player.setTexture("atlas", "misa-front");
    });
    this.input.keyboard.once("keyup_LEFT", function (event) {
      _this2.player.anims.stop();

      _this2.player.setTexture("atlas", "misa-left");
    });
    this.input.keyboard.once("keyup_RIGHT", function (event) {
      _this2.player.anims.stop();

      _this2.player.setTexture("atlas", "misa-right");
    }); // The lifting animations

    this.input.keyboard.on("keydown_L", function (event) {
      // If we were moving, pick and idle frame to use
      if (prevVelocity.x < 0) {
        _this2.player.anims.play("misa-left-lift", true);
      } else if (prevVelocity.x > 0) {
        _this2.player.anims.play("misa-right-lift", true);
      } else if (prevVelocity.y < 0) {
        _this2.player.anims.play("misa-back-lift", true);
      } else if (prevVelocity.y > 0) {
        _this2.player.anims.play("misa-front-lift", true);
      }
    }); // The lifting animations
    ...
    this.input.keyboard.on("keydown_ENTER", function (event) {
      ...
      if (prevVelocity.x < 0) {
        ...
        _this2.player.anims.play("misa-sword-swing-down", true);
      }
    });
  }
});